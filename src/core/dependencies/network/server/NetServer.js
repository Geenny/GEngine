...
import ServerStruct from "../struct/NetServerStruct";
...
import LoaderAbstract from "../loaders/LoaderAbstract";
import Log from "../../../../utils/log/Log";

export default class NetServer extends EventDispatcherVOWrapper {
    ...
    get isRequestQueueLimit() { return this.requestQueueCount <= 0 ||
        this._loaderStructSendedList.length >= this.requestQueueCount }
    ...
    loaderStructGet( loader ) {
        let loaderStruct = ArrayUtils.findAsObject( this._loaderStructList, "loader", loader ) ||
            ArrayUtils.findAsObject( this._loaderStructSendedList, "loader", loader );
        return loaderStruct;
    }

    /**
     * Определить является ли @LoaderAbstract посыльным для данного @Net
     * @param { LoaderAbstract } loader 
     */
    inLoader( loader ) {
    ...
    _loaderByClassCreate( LoaderClass ) {
        return ( LoaderClass instanceof LoaderAbstract ) ?
    ...
    _loaderStructRemove( loaderStruct ) {
        ...
        index = this._loaderStructSendedList.indexOf( loaderStruct );
        if ( index >= 0 ) this._loaderStructSendedList.splice( index, 1 );
    }
    ...
    _loaderSend() {
        ...
        this._loaderStructSendedList.push( loaderStruct );
